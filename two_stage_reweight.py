...
labelcnt = train_df['label'].value_counts()
cls_num_list = labelcnt.sort_index().tolist()

# Inverse of Number of Samples
def get_weights_inverse_num(cls_num_list, power=1):
    """ power = 1, vanila re-weighting, inverse of number of samples 
        power = 0.5, Inverse of square root of number of samples 
    """
    weights_per_cls = 1.0 / np.array(np.power(cls_num_list, power))
    weights_per_cls = weights_per_cls / np.sum(weights_per_cls) * len(cls_num_list)
    return weights_per_cls

def get_weights_effective_num(cls_num_list, beta = 0.9999):
    """ Class-balanced - effective number of samples (ens) """
...
weights_per_cls = get_weights_effective_num(cls_num_list)

# put weights to device
weights_per_cls = torch.FloatTensor(weights_per_cls).cuda()
...
for seed in seed_list:
    ...
    loss_stage_1 = LDAMLoss(cls_num_list=cls_num_list, max_m=0.5, s=1, weight=weights_per_cls)
